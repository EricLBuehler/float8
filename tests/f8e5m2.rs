...
mod test {
    ...
    #[test]
    fn as_primitive() {
        let two = F8E5M2::from_f32(2.0);
        assert_eq!(<i32 as AsPrimitive<F8E5M2>>::as_(2), two);
        assert_eq!(<F8E5M2 as AsPrimitive<i32>>::as_(two), 2);

        assert_eq!(<f32 as AsPrimitive<F8E5M2>>::as_(2.0), two);
        assert_eq!(<F8E5M2 as AsPrimitive<f32>>::as_(two), 2.0);

        assert_eq!(<f64 as AsPrimitive<F8E5M2>>::as_(2.0), two);
        assert_eq!(<F8E5M2 as AsPrimitive<f64>>::as_(two), 2.0);
    }
    ...
    fn from_primitive() {
        let two = F8E5M2::from_f32(2.0);
        assert_eq!(<F8E5M2 as FromPrimitive>::from_i32(2).unwrap(), two);
        assert_eq!(<F8E5M2 as FromPrimitive>::from_f32(2.0).unwrap(), two);
        assert_eq!(<F8E5M2 as FromPrimitive>::from_f64(2.0).unwrap(), two);
    }
    ...
    fn test_f8e5m2_consts() {
        ...
        let one_plus_epsilon = F8E5M2::from_bits(one.to_bits() + 1);
        let epsilon = F8E5M2::from_f32(one_plus_epsilon.to_f32() - 1.0);
        assert_eq!(F8E5M2::EPSILON, epsilon);
        // sanity check to show test is good
        ...
        let min = F8E5M2::from_bits(F8E5M2::NEG_INFINITY.to_bits() - 1);
        let min_pos = F8E5M2::from_f32(2f32.powi(F8E5M2::MIN_EXP - 1));
        assert_eq!(F8E5M2::MAX, max);
        ...
        assert!(ten_to_min / 10.0 < F8E5M2::MIN_POSITIVE.to_f32());
        assert!(ten_to_min > F8E5M2::MIN_POSITIVE.to_f32());
        let ten_to_max = 10f32.powi(F8E5M2::MAX_10_EXP);
    ...
    fn test_f8e5m2_consts_from_f32() {
        ...
        assert_eq!(F8E5M2::NEG_ZERO, neg_zero);
        ...
        assert_eq!(F8E5M2::FRAC_1_PI, frac_1_pi);
        assert_eq!(F8E5M2::FRAC_1_SQRT_2, frac_1_sqrt_2);
        assert_eq!(F8E5M2::FRAC_2_PI, frac_2_pi);
        assert_eq!(F8E5M2::FRAC_2_SQRT_PI, frac_2_sqrt_pi);
        assert_eq!(F8E5M2::FRAC_PI_2, frac_pi_2);
        assert_eq!(F8E5M2::FRAC_PI_3, frac_pi_3);
        assert_eq!(F8E5M2::FRAC_PI_4, frac_pi_4);
        assert_eq!(F8E5M2::FRAC_PI_6, frac_pi_6);
        assert_eq!(F8E5M2::FRAC_PI_8, frac_pi_8);
        assert_eq!(F8E5M2::LN_10, ln_10);
        assert_eq!(F8E5M2::LN_2, ln_2);
        assert_eq!(F8E5M2::LOG10_E, log10_e);
        assert_eq!(F8E5M2::LOG10_2, log10_2);
        assert_eq!(F8E5M2::LOG2_E, log2_e);
        assert_eq!(F8E5M2::LOG2_10, log2_10);
        assert_eq!(F8E5M2::SQRT_2, sqrt_2);
    }
    ...
    fn test_nan_conversion_to_smaller() {
        ...
        let nan8_from_64 = F8E5M2::from_f64(nan64);
        let neg_nan8_from_64 = F8E5M2::from_f64(neg_nan64);
        let nan8_from_32 = F8E5M2::from_f32(nan32);
        let neg_nan8_from_32 = F8E5M2::from_f32(neg_nan32);
    ...
    fn test_nan_conversion_to_larger() {
        let nan8 = F8E5M2::from_bits(0x7E);
        let neg_nan8 = F8E5M2::from_bits(0xFE);
        let nan32 = f32::from_bits(0x7F80_0001u32);
        let neg_nan32 = f32::from_bits(0xFF80_0001u32);
    ...
    fn test_f8e5m2_to_f32() {
        ...
        let f = F8E5M2::from_f32(7.1);
        let diff = (f.to_f32() - 7.1f32).abs();
        // diff must be <= 4 * EPSILON, as 7 has two more significant bits than 1
        assert!(diff <= 4.0 * F8E5M2::EPSILON.to_f32());

        assert_eq!(F8E5M2::from_bits(0x0000_0001).to_f32(), 2.0f32.powi(-16));
        assert_eq!(
            F8E5M2::from_bits(0x0000_0005).to_f32(),
            5.0 * 2.0f32.powi(-16)
        );

        assert_eq!(
            F8E5M2::from_bits(0x0000_0001),
            F8E5M2::from_f32(2.0f32.powi(-16))
        );
        assert_eq!(
            F8E5M2::from_bits(0x0000_0005),
            F8E5M2::from_f32(5.0 * 2.0f32.powi(-16))
        );
    }
    ...
    fn test_f8e5m2_to_f64() {
        ...
        // 7.1 is NOT exactly representable in 8-bit, it's rounded
        let f = F8E5M2::from_f64(7.1);
        let diff = (f.to_f64() - 7.1f64).abs();
        // diff must be <= 4 * EPSILON, as 7 has two more significant bits than 1
        assert!(diff <= 4.0 * F8E5M2::EPSILON.to_f64());

        assert_eq!(F8E5M2::from_bits(0x0000_0001).to_f64(), 2.0f64.powi(-16));
        assert_eq!(
            F8E5M2::from_bits(0x0000_0005).to_f64(),
            5.0 * 2.0f64.powi(-16)
        );

        assert_eq!(
            F8E5M2::from_bits(0x0000_0001),
            F8E5M2::from_f64(2.0f64.powi(-16))
        );
        assert_eq!(
            F8E5M2::from_bits(0x0000_0005),
            F8E5M2::from_f64(5.0 * 2.0f64.powi(-16))
        );
    }
    ...
    fn test_comparisons() {
        ...
        let one = F8E5M2::from_f64(1.0);
        let neg_zero = F8E5M2::from_f64(-0.0);
        let neg_one = F8E5M2::from_f64(-1.0);
    ...
    #[test]
    fn arithmetic() {
        assert_eq!(F8E5M2::ONE + F8E5M2::ONE, F8E5M2::from_f32(2.));
        assert_eq!(F8E5M2::ONE - F8E5M2::ONE, F8E5M2::ZERO);
        assert_eq!(F8E5M2::ONE * F8E5M2::ONE, F8E5M2::ONE);
        assert_eq!(
            F8E5M2::from_f32(2.) * F8E5M2::from_f32(2.),
            F8E5M2::from_f32(4.)
        );
        assert_eq!(F8E5M2::ONE / F8E5M2::ONE, F8E5M2::ONE);
        assert_eq!(
            F8E5M2::from_f32(4.) / F8E5M2::from_f32(2.),
            F8E5M2::from_f32(2.)
        );
        assert_eq!(
            F8E5M2::from_f32(4.) % F8E5M2::from_f32(3.),
            F8E5M2::from_f32(1.)
        );
    }
